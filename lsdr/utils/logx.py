...
import atexit
import os
from lsdr.utils.mpi_tools import proc_id, mpi_statistics_scalar
...
class Logger:
    ...
    def __init__(self, output_dir=None, output_fname='progress.txt',
                 exp_name=None, phase='train', console_out=False):
        """
        Initialize a Logger.
        ...
        if proc_id() == 0:
            ...
            if osp.exists(output_path) and output_fname == 'progress.txt':
                raise ValueError(
                    colorize(
                        'Experiment files %s already exists!' %
                        output_path,
                        'yellow',
                        bold=True))
    ...
    def save_config(self, config, append=False):
        ...
        if proc_id() == 0:
            ...
            output = json.dumps(
                config_json, separators=(
                    ',', ':\t'), indent=4, sort_keys=True)
            # print(colorize('Saving config:\n', color='cyan', bold=True))
            # print(output)
            if append:
                with open(osp.join(self.output_dir, "config.json"), 'a+') as out:
                    out.write(output)
            else:
    ...
    def setup_tf_saver(self, sess, inputs, outputs):
        ...
        self.tf_saver_info = {
            'inputs': {
                k: v.name for k, v in inputs.items()}, 'outputs': {
