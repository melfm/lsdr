from lsdr.user_config import DEFAULT_DATA_DIR, FORCE_DATESTAMP, \
    DEFAULT_SHORTHAND, WAIT_BEFORE_LAUNCH
from lsdr.utils.logx import colorize
from lsdr.utils.mpi_tools import mpi_fork, msg
from lsdr.utils.serialization_utils import convert_json
import base64
...
def setup_logger_kwargs(exp_name,
    ...
    Args:
        ...
        data_dir (string): Path to folder where results should be saved.
            Default is the ``DEFAULT_DATA_DIR`` in ``lsdr/user_config.py``.
    ...
    if phase == 'transfer':
        relpath = ''.join([ymd_time, exp_src])
    else:
        relpath = ''.join([ymd_time, exp_name])

    relpath = osp.join(relpath, phase)

    if phase == 'transfer':
        relpath = osp.join(relpath, exp_name)
...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None,
    ...
    Args:
        ...
        data_dir (string): Used in configuring the logger, to decide where
            ...
            default to ``DEFAULT_DATA_DIR`` from ``lsdr/user_config.py``.
    ...
    plot_cmd = 'python -m lsdr.run plot '+logger_kwargs['output_dir']
    plot_cmd = colorize(plot_cmd, 'green')

    test_cmd = 'python -m lsdr.run test_policy '+logger_kwargs['output_dir']
...
class ExperimentGrid:
    ...
    def add(self, key, vals, shorthand=None, in_name=False):
        ...
        ``lsdr/user_config.py`` file to ``False``.
    ...
    def variant_name(self, variant):
        ...
        for k, v, sh, inn in zip(self.keys, self.vals,
                                 self.shs, self.in_names):
            ...
            for k, v in var.items():
                if ':' in k:
                    ...
                    assert k0 not in new_var or isinstance(
                        new_var[k0], dict), "You can't assign multiple values to the same key."
    ...
    def run(self, thunk, num_cpu=1, data_dir=None, datestamp=False):
        ...
        if WAIT_BEFORE_LAUNCH > 0:
            ...
            prog_bar = trange(
                steps, desc='Launching in...', leave=False,
                ncols=DIV_LINE_WIDTH, mininterval=0.25,
